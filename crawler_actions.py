...
def update_archive(pool):
    ...
    for day in list(arch.keys()):
        items = sorted(arch[day], key=lambda x: (x.get("importance",0), x.get("date","")), reverse=True)
        arch[day] = dedupe_topics(items)[:50]
    # 최근 7일만 보관
    days = sorted(arch.keys(), reverse=True)[:7]
    arch = {d: arch[d] for d in days}
    with open("archive.json", "w", encoding="utf-8") as f:
        json.dump(arch, f, ensure_ascii=False)
    total = sum(len(v) for v in arch.values())
    print(f"중요 기사 보관함: {len(arch)}일 / {total}건")
    return arch
